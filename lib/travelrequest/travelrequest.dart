...
class _TravelRequestScreenState extends State<TravelRequestScreen> {
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  timeSlotWidget(),
                  bookingTypeWidget(),
                  Visibility(child: localAddressWidget(), visible: isHotel),
                  textWidget("From Detail"),
                  countryListWidget(),
                  stateListWidget(),
                  cityListWidget(),
                  Visibility(
                      child: textWidget("To Detail"),
                      visible: isHotel ? false : true),
                  Visibility(
                      child: countryToListWidget(),
                      visible: isHotel ? false : true),
                  Visibility(
                      child: stateToListWidget(),
                      visible: isHotel ? false : true),
                  Visibility(
                      child: cityToListWidget(),
                      visible: isHotel ? false : true),
                  remarkWidget(),
                  submitWidget()
                ],
  ...
  Future<void> getCityList(int pos) async {
    ...
    dynamic response = await HTTP.get(getCityListAPI(
        ...
        pos == 0 ? stateCodeSpinner! : stateToCodeSpinner!));
    if (response != null && response.statusCode == 200) {
      jsonData = convert.jsonDecode(response.body);
  ...
  Future<void> sendRequestToSap() async {
    ...
    travelRequest = TravelRequest(
      ...
      bookingType: sendBookingType!,
      empName: nameController.text.toString(),
      fromCountry: countryCodeSpinner!,
      fromDistr: cityCodeSpinner!,
      fromState: stateCodeSpinner!,
      toCountry: countryToCodeSpinner!,
      toDistr: cityToCodeSpinner!,
      toState: stateToCodeSpinner!,
      suggested: remarkController.text.toString(),
      travelFrom: fromCity!,
      travelTo: toCity!,
      departSlot: sendTimeSlot!,
      travelDateFrom: selectedFromDate.toString(),
      travelDateTo: selectedToDate.toString(),
    );
